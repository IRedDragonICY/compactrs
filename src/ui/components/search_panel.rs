...
use crate::ui::layout::{LayoutNode, SizePolicy, AlignItems, JustifyContent, FlexWrap};
use crate::w;
...
impl Component for SearchPanel {
    unsafe fn create(&mut self, parent: HWND) -> Result<(), String> { unsafe {
        ...
        crate::ui::wrappers::ComboBox::new(self.hwnd_combo_filter_col).set_selected_index(0);
...
impl SearchPanel {
    pub unsafe fn refresh_layout(&self) {
        ...
        let row1 = LayoutNode::row(6, 0)
            .justify_content(JustifyContent::SpaceBetween)
            .align_items(AlignItems::Center)
            .with(self.hwnd_search, Flex(1.0))
            .spacer(10)
            .with(self.hwnd_lbl_results, Fixed(200));

        let row2 = LayoutNode::row(8, 10)
        ...
        LayoutNode::col(0, 0)
            .with_child(row1.with_policy(Fixed(40)))
            .with_child(row2.with_policy(Flex(1.0)))
            .apply_layout(rect);
    }
}